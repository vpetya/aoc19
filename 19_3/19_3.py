...
def setElement(x,y,c):
    ...
    if not y in matrix[x]:
        matrix[x][y] = {'num':1,0:10000000,1:1000000}
    matrix[x][y]['num'] *= (c+2)
    matrix[x][y][c] = min(matrix[x][y][c],counter[c])
...
origo = 100000
matrix = {origo:{origo:{'num':6}}}

actual = [origo,origo]
counter = [0,1000000]

for f in first:
    nextpos = step(actual,f,0)

actual = [origo,origo]
counter = [1000000,0]
for s in second:
    nextpos = step(actual,s,1)

matrix[origo][origo]['num']-=1
#for i in matrix:
#    print(i)
...
for i in matrix:
    for j in matrix[i]:
        if matrix[i][j]['num'] > 0 and matrix[i][j]['num'] % 6 == 0:
