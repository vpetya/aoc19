...
def step(f, c):
    ...
    if f[0] == "D":
        for i in range(num):
            setElement(actual[0]+i+1, actual[1], c)
        actual[0] += num
    elif f[0] == "U":
        for i in range(num):
            setElement(actual[0]-i-1, actual[1], c)
        ...
        for i in range(num):
            setElement(actual[0], actual[1]+i+1, c)
        actual[1] += num
    else:
        for i in range(num):
            setElement(actual[0], actual[1]-i-1, c)
...
closest = fewest = 1000000000

for (dist, steps) in [(abs(j - origo) + abs(i - origo), matrix[i][j][0] + matrix[i][j][1]) for i in matrix for j in matrix[i] if matrix[i][j][0] * matrix[i][j][1] > 0]:
    if dist < closest:
        closest = dist
    if steps < fewest:
        fewest = steps
...
#             dist = abs(i-origo) + abs(j-origo)
#             if dist < closest:
#                 closest = dist
#             steps = matrix[i][j][0] + matrix[i][j][1]
#             if steps < fewest:
#                 fewest = steps
